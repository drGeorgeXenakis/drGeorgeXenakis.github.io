...
aciCurveGraphList <- fitAciCurves("graph")
...
## for(i in 1:5){
##     if(i==1){
##         b <- as.POSIXct((sDCT%>%select(timestamp)%>%head(1))[[1]])
##     }else{
##         b <- as.POSIXct(lightCurveDates[[i-1]])
##     }
##     if(i==5){
##         e <- as.POSIXct((sDCT%>%select(timestamp)%>%tail(1))[[1]])
##     }else{
##         e <- as.POSIXct(lightCurveDates[[i]])
##     }
##     l[[i]] <- sDCT%>%filter(between(timestamp,b,e))%>%
##         mutate_all(~ifelse(is.nan(.),NA,.))%>%
##         rename(efficiency=n)%>%
##         ## group_by(timestamp=as.POSIXct(cut(timestamp,"1 day")))%>%
##         group_by(treeNo)%>%
##         summarise(eff=mean(efficiency,na.rm=TRUE),
##                   eff.se=se.f(efficiency))%>%
##         mutate(startTimestamp=b,endTimestamp=e,.before=1)
## }

## meanEff.data <- do.call("rbind",l)

## meanEff.data <- tibble(treeNo=treeOrder,id=treeID)%>%right_join(meanEff.data,by="treeNo")

## ## meanEff.data <- meanEff.data%>%
## ##     mutate(treatment=case_when(treeNo %in% c(6,8,12)~"C",
## ##                                treeNo %in% c(2,5,16)~"MD"))%>%
## ##     mutate(treat=treatment,trN=treeNo)%>%
## ##     unite(id,c("treat","trN"),sep="-")%>%
## ##     mutate(date=as.Date(startTimestamp))
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ##
...
pred.ksat<-tibble(ksat=seq(0,30,2),n.pred=Michaelis_Menten(ksat,slope=coef(fit.ksat)[[2]],sat=coef(fit.ksat)[[1]],r=coef(fit.ksat)[[3]]))
...
pred.phiJ<-tibble(phiJ=seq(0,30,2),n.pred=Michaelis_Menten(phiJ,slope=coef(fit.phiJ)[[2]],sat=coef(fit.phiJ)[[1]],r=coef(fit.phiJ)[[3]]))
...
pred.vcmax<-tibble(vcmax=seq(0,30,2),n.pred=Michaelis_Menten(vcmax,slope=coef(fit.vcmax)[[2]],sat=coef(fit.vcmax)[[1]],r=coef(fit.vcmax)[[3]]))
...
pred.jmax<-tibble(jmax=seq(0,30,2),n.pred=Michaelis_Menten(jmax,slope=coef(fit.jmax)[[2]],sat=coef(fit.jmax)[[1]],r=coef(fit.jmax)[[3]]))
...
#   plotAesthetic+
#   geom_line(data=pred,aes(x=ksat,y=n.pred))
